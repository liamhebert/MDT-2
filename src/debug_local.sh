...
srun python train.py experiment=giga_pretrain_roberta_anchor \
    logger=wandb env=all \
    env=all \
    hydra/launcher=submitit_slurm \
    trainer=gpu trainer.max_epochs=3 trainer.min_epochs=3 \
    dataset.dataset.debug=5
