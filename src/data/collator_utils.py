...
def extract_and_merge_features(
    ...
    for graph in graphs:
        ...
        if "edges" in graph:
            graph_features[GraphFeatures.Edges].append(graph["edges"])
        # else:
        #     del graph_features[GraphFeatures.Edges]
...
def generic_collator(
    ...
    num_padding = block_size - ((total_num_nodes) % block_size)
    ...
    for key, value in text_features.items():
        ...
        orig_shape = collated_text_features[key].shape
    ...
    sizes = [len(i) for i in out_degrees]
    graph_sizes = torch.tensor(sizes)
    cu_seqlens = torch.cumsum(graph_sizes, dim=0)
    max_seqlen = max(sizes)
    cu_seqlens = torch.cat((torch.tensor([0]), cu_seqlens), dim=0).to(
        torch.int32
    )
    ...
    # )
    rotary_pos = torch.cat([rotary_pos, virtual_distance_pad], dim=0)
    ...
    total_nodes = out_degree.shape[0]
    ...
    # )
    spatial_pos = torch.cat((spatial_pos, virtual_distance_pad), dim=0)
    indexing_pos = torch.cat((indexing_pos, virtual_distance_pad), dim=0)
    ...
    # )
    spatial_pos = torch.cat(
        (spatial_pos, virtual_distance_pad.T),
        dim=1,
    )
    indexing_pos = torch.cat(
        (indexing_pos, virtual_distance_pad.T),
        dim=1,
    )
...
def pyg_collator(
    ...
    Returns:
        ...
        - text_input (dict): batched text inputs, with the following keys:
            - input_ids (torch.Tensor): batched tokenized text ids, with shape
                (batch_size * N, T), where N is the number of nodes in the graph
                and T is the maximum text length.
            - token_type_ids (torch.Tensor): batched token type ids, with shape
                (batch_size * N, T), where N is the number of nodes in the graph
                and T is the maximum text length.
            - attention_mask (torch.Tensor): batched text attention mask, with
                shape (batch_size * N, T), where 1 indicates a token that should
                be attended to and 0 indicates padding.
        - edge_index (torch.Tensor): batched edge indices, with shape
            (E, E), where E is the number of edges in the graph, linking
            batch_size * N nodes to each other.
    """
    # As we try out newer models, we may need to change this as we verify them.
    assert all(
        all(isinstance(i, torch.Tensor) for i in value)
    ...
    graph_edges = graph_features[GraphFeatures.Edges]
    current_max = 0
    for edges in graph_edges:
        edges += current_max
        current_max = edges.max() + 1

    collated_graph_edges = torch.cat(graph_edges, dim=0)
    return {
        "text_input": collated_text_features,
        "edge_index": collated_graph_edges,
        "num_total_graphs": len(graph_edges),
        "num_total_nodes": collated_graph_edges.shape[0],
    }
