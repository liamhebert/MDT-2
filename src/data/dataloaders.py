...
from data.collator_utils import collator
from data.dataset import GraphormerPYGDataset
...
class GraphormerDataset:
    ...
    NOTE: This class is a product of legacy, as we used to have multiple Dataset
    providers (PyG, DGL, etc). As a result, this is not totally necessary since
    we only use PyG datasets.
    """

    # TODO(liamhebert): Merge this with BatchedDataDataset

    def __init__(self, dataset: GraphormerPYGDataset):
        """Initializes the dataset wrapper.

        Args:
            dataset (GraphormerPYGDataset): Dataset to pull from.
        """
        super().__init__()
...
class BatchedDataDataset(ABC, Dataset):
    """
    ...
    def __getitem__(self, index: int) -> Data:
        ...
        Returns:
            Data: Single processed graph object
        """
...
# TODO(liamhebert): I have no idea what this class is used for
...
#         # We shuffle the indices for each epoch
